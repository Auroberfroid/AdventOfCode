...
for to_process_sequence in data_input:
    ...
    # Append a 0 at the end of the list of 0
    sequences[-1].append(0)
    # Compute last int of the the second to last list --> Append the value to its left
    sequences[-2].append(sequences[-2][-1])
    ...
    for index_sequence in range(-3, -(n+1), -1):
        #index_sequence = n - _index_sequence - 3
        sequences[index_sequence].append(sequences[index_sequence][-1] + sequences[index_sequence+1][-1])
